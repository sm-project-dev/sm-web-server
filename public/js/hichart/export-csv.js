...
(function(Highcharts) {
  ...
  Highcharts.Chart.prototype.getDataRows = function() {
    const options = (this.options.exporting || {}).csv || {};
    ...
    // Options
    const dateFormat = options.dateFormat || '%Y-%m-%d %H:%M:%S';
    const columnHeaderFormatter =
    ...
    each(this.series, function(series) {
      const { keys } = series.options;
      const pointArrayMap = keys || series.pointArrayMap || ['y'];
      const valueCount = pointArrayMap.length;
      const { requireSorting } = series;
      let j;
      ...
      if (series.options.includeInCSVExport !== false && series.visible !== false) {
        ...
        each(series.points, function(point, pIdx) {
          const key = requireSorting ? point.x : pIdx;
  ...
  Highcharts.Chart.prototype.getCSV = function(useLocalDecimalPoint) {
    ...
    const options = (this.options.exporting || {}).csv || {};
    const itemDelimiter = options.itemDelimiter || ','; // use ';' for direct import to Excel
    ...
    each(rows, function(row, i) {
      let val = '';
      let j = row.length;
      const n = useLocalDecimalPoint ? (1.1).toLocaleString()[1] : '.';
      while (j--) {
        val = row[j];
    ...
    each(rows, function(row, i) {
      ...
      for (j = 0; j < row.length; j += 1) {
        ...
        if (typeof val === 'number') {
          if (n === ',') {
            html += `<${tag}${
              typeof val === 'number' ? ' class="number"' : ''
            }>${val.toString().replace('.', ',')}</${tag}>`;
          } else {
            html += `<${tag}${
              typeof val === 'number' ? ' class="number"' : ''
            }>${val.toString()}</${tag}>`;
  ...
  function getContent(chart, href, extension, content, MIME) {
    ...
    let blobObject;
    let name;
    const options = (chart.options.exporting || {}).csv || {};
    const url = options.url || 'http://www.highcharts.com/studies/csv-export/download.php';
  ...
  Highcharts.Chart.prototype.downloadCSV = function() {
    ...
    getContent(this, `data:text/csv,${csv.replace(/\n/g, '%0A')}`, 'csv', csv, 'text/csv');
  };
  ...
  Highcharts.Chart.prototype.downloadXLS = function() {
    ...
    const template = `${'<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40">' +
      ...
      '<meta name=ProgId content=Excel.Sheet /><meta chartset=UTF-8/>' +
      '</head><body>'}${this.getTable(true)}</body></html>`;
